...
const CanvasLayer = (L.Layer ? L.Layer : L.Class).extend({
  ...
  _onLayerDidResize(resizeEvent) {
    this._setCanvasSize(resizeEvent.newSize.x, resizeEvent.newSize.y);
  },
  ...
  onAdd(map) {
    ...
    this._setCanvasSize(size.x, size.y);
    ...
    if (del.onLayerDidMount) {
      del.onLayerDidMount(); // -- callback
    }
    this.needRedraw();
  },
  ...
  onRemove(map) {
    const del = this._delegate || this;
    if (del.onLayerWillUnmount) {
      del.onLayerWillUnmount(); // -- callback
    }
